#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into the local opencode skills dir."""
...
def _prepare_repo(source: Source, method: str, tmp_dir: str) -> str:
    if method != "download":
        raise InstallError("Only download mode is supported. Git commands are disabled.")
    try:
        ...
        if "HTTP 401" in message or "HTTP 403" in message or "HTTP 404" in message:
            raise InstallError(
                f"{message}. Provide GITHUB_TOKEN/GH_TOKEN or manually supply the skill files."
            ) from exc
        raise

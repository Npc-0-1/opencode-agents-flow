---
name: skill-creator
description: Guide for creating effective opencode-compatible skills. Use when creating or updating a single skill with specialized knowledge, workflows, or tool integrations. Do not use for full skills directory scanning, governance, registration consistency, or responsibility-overlap audits; use opencode-skill-designer instead.
metadata:
...
- Before creating a skill, scan existing skills and merge overlapping behavior instead of creating duplicates.
- Prefer one stronger skill over several near-identical commands.
- `description` must say what the skill does and when to trigger it; front-load concrete keywords, file types, and user phrases.
...
- **Benefits**: Separates output resources from documentation, enables another compatible agent to use files without loading them into context
...
When editing the (newly-generated or existing) skill, remember that the skill is being created for another compatible agent instance to use. Include information that would be beneficial and non-obvious to that agent. Consider what procedural knowledge, domain-specific details, or reusable assets would help another compatible agent instance execute these tasks more effectively.
...
For opencode skills, keep `name` and `description` required. Optional `license`, `compatibility`, and `metadata` are allowed when they have a clear purpose; do not add other frontmatter fields.
